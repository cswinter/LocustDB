...
impl<T> Aggregator<T, i64> for SumI64
where
    T: Into<i64>,
{
    fn unit() -> i64 {
...
impl<T> CheckedAggregator<T, i64> for SumI64
where
    T: Into<i64>,
{
    #[inline]
    fn accumulate_checked(acc: i64, value: T) -> (i64, bool) {
        acc.overflowing_add(value.into())
    }
    #[inline]
    fn combine_checked(acc1: i64, acc2: i64) -> (i64, bool) {
        acc1.overflowing_add(acc2)
    }
}

pub struct SumF64;

impl<T> Aggregator<T, OrderedFloat<f64>> for SumF64
where
    T: Into<OrderedFloat<f64>>,
{
    fn unit() -> OrderedFloat<f64> {
        OrderedFloat(0.0)
    }
    #[inline]
    ...
    #[inline]
    fn combine(
        accumulator1: OrderedFloat<f64>,
        accumulator2: OrderedFloat<f64>,
    ) -> OrderedFloat<f64> {
        accumulator1 + accumulator2
    }
}
...
impl<V> Aggregator<V, u32> for Count {
    ...
    #[inline]
    fn accumulate(accumulator: u32, _: V) -> u32 {
        accumulator + 1
    }
    #[inline]
    fn combine(accumulator1: u32, accumulator2: u32) -> u32 {
        accumulator1 + accumulator2
    }
}
...
impl<V> Aggregator<V, i64> for MaxI64
where
    V: Into<i64>,
{
    fn unit() -> i64 {
    ...
    #[inline]
    fn accumulate(accumulator: i64, value: V) -> i64 {
        std::cmp::max(accumulator, value.into())
    }
    #[inline]
    fn combine(accumulator1: i64, accumulator2: i64) -> i64 {
        std::cmp::max(accumulator1, accumulator2)
    }
}

pub struct MaxF64;

impl<V> Aggregator<V, OrderedFloat<f64>> for MaxF64
where
    V: Into<OrderedFloat<f64>>,
{
    ...
    #[inline]
    fn accumulate(accumulator: OrderedFloat<f64>, value: V) -> OrderedFloat<f64> {
        std::cmp::max(accumulator, value.into())
    }
    #[inline]
    fn combine(
...
impl<V> Aggregator<V, i64> for MinI64
where
    V: Into<i64>,
{
    fn unit() -> i64 {
    ...
    #[inline]
    fn accumulate(accumulator: i64, value: V) -> i64 {
        std::cmp::min(accumulator, value.into())
    }
    #[inline]
    fn combine(accumulator1: i64, accumulator2: i64) -> i64 {
        std::cmp::min(accumulator1, accumulator2)
    }
}

pub struct MinF64;

impl<V> Aggregator<V, OrderedFloat<f64>> for MinF64
where
    V: Into<OrderedFloat<f64>>,
{
    fn unit() -> OrderedFloat<f64> {
...
impl<'a, T, U, V, A: Aggregator<T, V>> VecOperator<'a> for Aggregate<T, U, V, A>
where
    T: VecData<T> + 'static,
    U: GenericIntVec<U>,
    V: VecData<V> + 'static,
...
impl<'a, T, U, V, A: Aggregator<T, V>> VecOperator<'a> for AggregateNullable<T, U, V, A>
where
    T: VecData<T> + 'static,
    U: GenericIntVec<U>,
    V: VecData<V> + 'static,
{
    fn execute(&mut self, _: bool, scratchpad: &mut Scratchpad<'a>) -> Result<(), QueryError> {
        ...
        if len > accumulators.len() {
            accumulators.resize(len, A::unit());
            accumulators_present.resize(len.div_ceil(8), 0);
        }
...
impl<'a, T, U, V, A: CheckedAggregator<T, V>> VecOperator<'a> for CheckedAggregate<T, U, V, A>
where
    T: GenericIntVec<T> + Into<V>,
    U: GenericIntVec<U>,
    V: GenericIntVec<V>,
{
    fn execute(&mut self, _: bool, scratchpad: &mut Scratchpad<'a>) -> Result<(), QueryError> {
...
impl<'a, T, U, V, A: CheckedAggregator<T, V>> VecOperator<'a>
    for CheckedAggregateNullable<T, U, V, A>
where
    T: GenericIntVec<T> + Into<i64>,
    U: GenericIntVec<U>,
    V: GenericIntVec<V>,
{
    fn execute(&mut self, _: bool, scratchpad: &mut Scratchpad<'a>) -> Result<(), QueryError> {
