...
#[derive(Debug)]
pub struct MergeDeduplicatePartitioned<T, C> {
...
impl<'a, T: VecData<T> + 'a, C: Comparator<T>> VecOperator<'a> for MergeDeduplicatePartitioned<T, C> {
    fn execute(&mut self, _: bool, scratchpad: &mut Scratchpad<'a>) -> Result<(), QueryError> {
        let (deduplicated, merge_ops) = {
            ...
            merge_deduplicate_partitioned::<_, C>(&partitioning, &left, &right)
        };
    ...
    fn outputs(&self) -> Vec<BufferRef<Any>> {
        vec![self.deduplicated.any(), self.merge_ops.any()]
    }
...
pub fn merge_deduplicate_partitioned<'a, T: Copy + PartialEq + 'a, C: Comparator<T>>(
    ...
    for group in partitioning {
        ...
        for _ in 0..(group.left + group.right) {
            ...
            if j < j_max && last == Some(right[j]) {
                ...
                j += 1;
            } else if i < i_max && (j >= j_max || C::cmp_eq(left[i], right[j])) {
                result.push(left[i]);
