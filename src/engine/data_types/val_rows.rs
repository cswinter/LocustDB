...
impl<'a> ValRows<'a> {
    pub fn new(row_len: usize) -> ValRows<'a> {
        ValRows { row_len, data: Vec::default() }
    }

    pub fn with_capacity(row_len: usize, rows: usize) -> ValRows<'a> {
        ValRows { row_len, data: Vec::with_capacity(row_len * rows) }
    }
...
impl<'a> Data<'a> for ValRows<'a> {
    ...
    fn get_type(&self) -> EncodingType { EncodingType::ValRows }
    ...
    fn type_error(&self, func_name: &str) -> String { format!("ValRows.{}", func_name) }
    ...
    fn cast_ref_mut_val_rows(&mut self) -> &mut ValRows<'a> { self }
}
