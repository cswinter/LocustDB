...
impl QueryTask {
    pub fn new(mut query: Query, explain: bool, show: Vec<usize>, source: Vec<Arc<Partition>>, sender: SharedSender<QueryResult>) -> QueryTask {
    ...
    pub fn run(&self) {
        ...
        while let Some((partition, id)) = self.next_partition() {
            ...
            let cols = partition.get_cols(&self.referenced_cols);
